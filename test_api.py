...
def test_guardrails():
    """Testa gerenciamento de guardrails"""
    print("\n🧪 Testando gerenciamento de guardrails...")
    
    # Teste 1: Listar guardrails
    try:
        response = requests.get(f"{BASE_URL}/guardrails")
        if response.status_code == 200:
            guardrails = response.json()
            print(f"✅ Listar guardrails: {len(guardrails)} guardrails encontrados")
            for guardrail in guardrails[:3]:  # Mostrar apenas os 3 primeiros
                enabled_status = "🟢" if guardrail.get('enabled', False) else "🔴"
                print(f"   - {enabled_status} {guardrail.get('name', 'N/A')}: {guardrail.get('description', 'N/A')}")
        else:
            print(f"❌ Listar guardrails: {response.status_code}")
    except Exception as e:
        print(f"❌ Listar guardrails: Erro - {e}")
    
    # Teste 2: Buscar guardrail específico
    try:
        response = requests.get(f"{BASE_URL}/guardrails/TransformersGuardrail")
        if response.status_code == 200:
            guardrail = response.json()
            print(f"✅ Buscar TransformersGuardrail: {guardrail.get('name', 'N/A')}")
        elif response.status_code == 404:
            print("⚠️ TransformersGuardrail não encontrado")
        else:
            print(f"❌ Buscar TransformersGuardrail: {response.status_code}")
    except Exception as e:
        print(f"❌ Buscar TransformersGuardrail: Erro - {e}")
